...
def detect_or_load_qr_codes(image_path: Path, qrcode_path: Path):
    """
...
def detect_qr_codes(capture: Capture, session_id: str):
    """
    ...
    for ts, cam_id in session.images.key_pairs():
        ...
        pool.apply_async(
            detect_or_load_qr_codes, args=(image_path, qrcode_path)
        )
    pool.close()
    pool.join()
...
def qrcode_detection(
    ...
    detect_qr_codes(capture, session_id)
